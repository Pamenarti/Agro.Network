...
HTTPS_GIT := https://github.com/fetchai/agrod.git
...
ldflags = -X github.com/cosmos/cosmos-sdk/version.Name=fetch \
		  -X github.com/cosmos/cosmos-sdk/version.ServerName=agrod \
...
build: go.sum
ifeq ($(OS),Windows_NT)
	go build -mod=readonly $(BUILD_FLAGS) -o build/agrod.exe ./cmd/agrod
else
	go build -mod=readonly $(BUILD_FLAGS) -o build/agrod ./cmd/agrod
endif
...
install: go.sum
	go install -mod=readonly $(BUILD_FLAGS) ./cmd/agrod
...
draw-deps:
	...
	@goviz -i ./cmd/agrod -d 2 | dot -Tpng -o dependency-graph.png
...
build-docker-agrodnode:
	$(MAKE) -C networks/local

# Run a 4-node testnet locally
localnet-start: build-linux localnet-stop
	@if ! [ -f build/node0/agrod/config/genesis.json ]; then docker run --rm -v $(CURDIR)/build:/agrod:Z tendermint/agrodnode testnet --v 4 -o . --starting-ip-address 192.168.10.2 ; fi
	docker-compose up -d
