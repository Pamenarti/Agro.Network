...
The simplest way to try out CosmWasm is simply to run `agrod` out of the box,
...
you may consider simply forking `agrod`. *I highly advise against this*. 
...
The simplest way to use `agrod` is just to import `x/wasm` and wire it up
...
at [`agrod/app/app.go`](https://github.com/fetchai/agrod/blob/master/app/app.go#)
...
from upstream on all future `agrod` releases, and this should be as simple
as possible.
...
power without forking either CosmWasm or `agrod`. 
...
If you look at the unit tests in [`x/wasm/internal/keeper`](https://github.com/fetchai/agrod/tree/master/x/wasm/internal/keeper),
...
[`CustomQuerier`](https://github.com/fetchai/agrod/blob/v0.8.0-rc1/x/wasm/internal/keeper/query_plugins.go#L35),
which allows you to handle your custom queries. The second is 
[`CustomEncoder`](https://github.com/fetchai/agrod/blob/v0.8.0-rc1/x/wasm/internal/keeper/handler_plugin.go#L30)
...
In particular, here [we define a `CustomQuerier`](https://github.com/fetchai/agrod/blob/v0.8.0-rc1/x/wasm/internal/keeper/reflect_test.go#L355-L385),
and here [we define a `CustomHandler`](https://github.com/fetchai/agrod/blob/v0.8.0-rc1/x/wasm/internal/keeper/reflect_test.go#L303-L353).
...
[encode staking messages](https://github.com/fetchai/agrod/blob/v0.8.0-rc1/x/wasm/internal/keeper/handler_plugin.go#L114-L192).
Note that withdraw returns 2 messages, which is an option you can use if needed to translate into native messages.
When we [handle staking queries](https://github.com/fetchai/agrod/blob/v0.8.0-rc1/x/wasm/internal/keeper/query_plugins.go#L109-L172)
...
[define the supported feature set](https://github.com/fetchai/agrod/blob/ade03a1d39a9b8882e9a1ce80572d39d57bb9bc3/x/wasm/internal/keeper/reflect_test.go#L52)
to include your custom name (remember `requires_XYZ` above?). Then, when creating `TestInput`, 
you can [pass in your custom encoder and querier](https://github.com/fetchai/agrod/blob/ade03a1d39a9b8882e9a1ce80572d39d57bb9bc3/x/wasm/internal/keeper/reflect_test.go#L52).
...
Just edit [the default `NewKeeper` constructor](https://github.com/fetchai/agrod/blob/v0.8.0-rc1/app/app.go#L257-L258)
