...
agrod config chain-id "${CHAINID}"
agrod config keyring-backend test

# setup the node with a default genesis
if [ ! -f "/root/.agrod/config/genesis.json" ]; then
	echo 'Generating genesis file...'
    agrod init "${MONIKER}" --chain-id "${CHAINID}" > /dev/null 2>&1
fi

# create the key if needed
agrod keys show "${MONIKER}" < passphrase.txt > /dev/null 2>&1 || agrod keys add "${MONIKER}" --recover < mnemonic-setup.txt > /dev/null 2>&1

# get the address
node_address=$(agrod keys show ${MONIKER} -a < passphrase.txt)
...
if [ ! -f /setup/genesis.json ]; then
	...
	if [ ! -f /setup/${node_address}.networkaddr ]; then
		node_id=$(agrod tendermint show-node-id)
	...
	cp /setup/genesis.intermediate.json /root/.agrod/config/genesis.json

	# generate the tx
	if [ ! -f "/setup/gentx-${node_address}.json" ]; then
		agrod gentx ${MONIKER} 1000000000000000000atestfet --chain-id "${CHAINID}" --output-document /setup/gentx-${node_address}.json < passphrase4.txt
	fi
...
# copy the generated genesis file
cp /setup/genesis.json /root/.agrod/config/genesis.json
