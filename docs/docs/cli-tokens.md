...
Once `agrod` is configured for the desired [network](../cli-introduction/). The user can query their balance using the following command:

```bash
agrod query bank balances fetch1akvyhle79nts4rwn075t85xrwmp5ysuqynxcn4
```
...
```bash
agrod tx bank send <from address or key name> <target address> <amount>
```
...
```bash
agrod tx bank send main fetch106vm9q6ezu9va7v7e0cvq0nedc54egjm692fcp 100atestfet
```
