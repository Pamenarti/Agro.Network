...
After running the command `agrod` will print out a summary of the new key. An example of this output is shown below:
...
```bash
agrod keys show -p <name of the key>
```
...
In order to use the hardware wallet address with the cli, the user must first add it via `agrod`. This process only records the public information about the key.
...
```bash
agrod keys add <name for the key> --ledger --index 0
```
