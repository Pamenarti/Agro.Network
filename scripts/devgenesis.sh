...
if unspecified, node_home_dir default to the ~/.agrod/ folder. 
this folder must exists and contains the files created by the "agrod init" command.

The updated genesis will be written under node_home_dir/config/genesis.json, allowing
the local chain to be started with:

agrod --home <node_home_dir> unsafe-reset-all && agrod --home <node_home_dir> start
...
NEW_TMADDR=$(agrod --home "${OUT_HOMEDIR}" tendermint show-address)
...
echo
echo "agrod --home ${OUT_HOMEDIR} unsafe-reset-all && agrod --home ${OUT_HOMEDIR} start"
echo
